"""
Event tensor generation and processing utilities.
Contains functions for building and processing event tensors.
"""
...
def process_window(
    i: int,
    events_cpu: Dict[str, np.ndarray],
    ...
    Args:
        i (int): Frame index
        events_cpu (dict): Event data on CPU
        frame_starts (np.ndarray): Frame start timestamps
        kernel_depth (int): Depth of the temporal kernel
        diffuse_time (float): Time window for diffusion
        event_step (int): Temporal subsampling factor
        height (int): Optional height for spatial clipping
        width (int): Optional width for spatial clipping
        dirname (str): Directory to save processed windows
    """
    frame_start = frame_starts[i]
    save_path = os.path.join(dirname, f"window_{i:04d}.pt")
